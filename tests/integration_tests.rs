...
fn test_sort_and_deduplicate() {
    ...
    debug_cmd.pipe_stdin(file.path()).unwrap().arg("--host");
    ...
    assert_eq!(lines[0], "a", "First line should be subdomain 'a'");
    assert_eq!(lines[1], "b", "Second line should be subdomain 'b'");
}
...
fn test_stdin_processing() {
    ...
    cmd.pipe_stdin(file.path()).unwrap().arg("--domain");
...
fn test_all_flag() {
    ...
    cmd.assert()
        ...
        .stdout(predicate::function(|s: &str| {
            s.contains("user") && s.contains("8080") && s.contains("www.example.com")
        }));
}
